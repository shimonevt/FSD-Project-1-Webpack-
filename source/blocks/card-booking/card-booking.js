...
jQuery(function($){
	...
	var RangeDatePicker = $(cardStart).datepicker({
		language : 'my-lang',
		range : true,
		clearButton : true,
		todayButton : true,
	...
	function getDateForCard(firstDate,secondDate) {
		let day_1 = new Date(parseInt(firstDate.slice(6)), firstDate.slice(3,5)-1, parseInt(firstDate.slice(0,2))),
    	day_2 = new Date(parseInt(secondDate.slice(6)), secondDate.slice(3,5)-1, parseInt(secondDate.slice(0,2)));
		
		
		let daysDiffer = (day_2 - day_1) / (60 * 60 * 24 * 1000);
		let roomCosts = document.querySelector('.room__description');
		let costPerDay = document.querySelector('.info__price > p.cost');
		...
		let temp = costPerDay.innerHTML.replace(/\s+/g, '')*daysDiffer;
